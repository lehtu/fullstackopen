...
class App extends React.Component {
  ...
  getMostPopularIndex = () => {
    ...
    Object.keys(this.state.votes).forEach((key) => {
      const voteCount = this.state.votes[key]
      if (voteCount > winner) {
        winner = voteCount
        winnerKey = key
      }
  ...
  render() {
    const index = this.getMostPopularIndex();
    const mostPopularAnecdote = this.props.anecdotes[index]
    const mostPopularVotes = this.state.votes[index]
